...
def _set_provider_keys(monkeypatch) -> None:
...
def test_fallback_execution(monkeypatch, tmp_path) -> None:
    _set_provider_keys(monkeypatch)
    transport = FakeRealTransport(failures={"openai": "provider_http_error"}, text="Fallback documentation generated.")
    result = _engine(tmp_path, transport).execute(_payload())
    assert result["execution_state"] == "degraded_mode"
    assert result["provider_used"] == "anthropic"
    assert result["fallback_provider_used"] == "anthropic"
...
def test_timeout_handling(monkeypatch, tmp_path) -> None:
    _set_provider_keys(monkeypatch)
    result = _engine(tmp_path, FakeRealTransport(failures={"openai": "provider_timeout"})).execute(
